...
namespace Paroxe.PdfRenderer
{
    ...
    public class PDFRenderer : IDisposable
    {
        ...
        private byte[] m_IntermediateBuffer2;
#endif
            ...
            IPDFColoredRectListProvider rectsProvider, RenderSettings settings)
        {
            ...
            if (m_IntermediateBuffer2 == null || m_IntermediateBuffer2.Length < length)
                m_IntermediateBuffer2 = new byte[width * height * 4];

            Marshal.Copy(bufferPtr, m_IntermediateBuffer2, 0, width * height * 4);
            ...
            if (coloredRects != null && coloredRects.Count > 0)
            {
                ...
                {
                    var rp = (int)(coloredRect.Color.r * 255) & 0xFF;
                    var gp = (int)(coloredRect.Color.g * 255) & 0xFF;
                    var bp = (int)(coloredRect.Color.b * 255) & 0xFF;
                    var ap = (int)(coloredRect.Color.a * 255) & 0xFF;

                    float alphap = (ap / (float)255);
                    float reverseAlphap = 1.0f - alphap;

                    Rect deviceRectp = page.ConvertPageRectToDeviceRect(coloredRect.PageRect, new Vector2(width, height));
                    for (int y = 0; y <= -(int)deviceRectp.height; ++y)
                    {
                        for (int x = 0; x <= (int)deviceRectp.width; ++x)
                        {
                            if ((x < 2) || (x > (int)deviceRectp.width - 2) || (y < 2) || (y > (-(int)deviceRectp.height) - 2))
                            {
                                int s = (((int)deviceRectp.y + y + (int)deviceRectp.height) * width + (int)deviceRectp.x + x) * 4;

                                var sr = m_IntermediateBuffer[s];
                                var sg = m_IntermediateBuffer[s + 1];
                                var sb = m_IntermediateBuffer[s + 2];

                                m_IntermediateBuffer[s] = (byte)Mathf.Clamp(alphap * rp + (reverseAlphap * sr), 0, 255);
                                m_IntermediateBuffer[s + 1] = (byte)Mathf.Clamp(alphap * gp + (reverseAlphap * sg), 0, 255);
                                m_IntermediateBuffer[s + 2] = (byte)Mathf.Clamp(alphap * bp + (reverseAlphap * sb), 0, 255);
            ...
            //IList<PDFColoredRect> coloredRects2 = rectsProvider != null
            //    ? rectsProvider.GetBackgroundColoredRectListForHighlight(page)
            //    : null;

            IList<PDFColoredRect> coloredRects2 = PDFViewerHelper.GetBackgroundColoredRectListForHighlight(page);


            if (coloredRects2 != null && coloredRects2.Count > 0)
            {
                foreach (PDFColoredRect coloredRect in coloredRects2)
                {

                    if (coloredRect.whole == true)//Bounding box
                    {
                        if (coloredRect.updating == true)//outline
                        {
                            var rp = (int)(coloredRect.Color.r * 255) & 0xFF;
                            var gp = (int)(coloredRect.Color.g * 255) & 0xFF;
                            var bp = (int)(coloredRect.Color.b * 255) & 0xFF;
                    ...
                    else
                    {
                        ...
                        for (int y = 0; y < -(int)deviceRect.height; ++y)
                        {
                            for (int x = 0; x < (int)deviceRect.width; ++x)
                            {
                                ...
                                var sr = m_IntermediateBuffer2[s];
                                var sg = m_IntermediateBuffer2[s + 1];
                                var sb = m_IntermediateBuffer2[s + 2];

                                m_IntermediateBuffer[s] = (byte)Mathf.Clamp(sr - (255 - r), 0, 255);
                                m_IntermediateBuffer[s + 1] = (byte)Mathf.Clamp(sg - (255 - g), 0, 255);
                                m_IntermediateBuffer[s + 2] = (byte)Mathf.Clamp(sb - (255 - b), 0, 255);
                                m_IntermediateBuffer[s + 3] = 0xFF;
                            }
                        }
                    }
